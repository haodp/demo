...
var judFloat = function(str) {
	var reg = /^[+|-]?\d*\.?\d*$/;
	if (!reg.test(str)) {
		$.extMessager.popup("E", "输入格式有误");
		return false;
	}
	else {
		return true;
...
(function(factory) {
	...
	Spinning.prototype = {
			...
			value: function(v){
				...
				if(valid !== 0){
					$.extMessager.popup("E", "输入值不合法<br>请依照提示的范围填写");
					return this.$el.val(null);
				}

				v = v.toFixed(this.options.precision);
				if (v * (1 / this.options.step) % 1 != 0) {
					$.extMessager.popup("E", "输入值不合法<br>请依照提示的调节精度填写");
					return this.$el.val(null);
				}
				this.$el.val(Number(v).toFixed(this.options.precision));
	...
	$.fn.spinner = function(options, value) {
		return this.each(function() {
			...
			data.spinning.value(options.value);
